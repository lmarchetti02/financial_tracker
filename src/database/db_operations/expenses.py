...
def fetch_expenses(
    ...
    return fetch_rows(location, EXPENSES_DB_NAME, Expense, sort=sort, month=month, extra_filter=extra_filter)


def fetch_category(location: DbLocation, category: str) -> np.ndarray:
    """Fetch the total expense per month for a specified category.

    Args:
        location (`:class:DbLocation`): The year/profile of the expenses.
    ...
    return fetch_monthly_totals(location, EXPENSES_DB_NAME, "cost", "category", category)


def fetch_expense_totals(location: DbLocation) -> np.ndarray:
    ...
    return fetch_monthly_totals(location, EXPENSES_DB_NAME, "cost")
